...
import type { LearningUnitCardProps } from "@/types/learning";

const LearningUnitCard: React.FC<LearningUnitCardProps> = ({
  unit,
  userProgress,
  isPreviousUnitCompleted,
}) => {
  ...
  return (
    ...
    >
      <Card
        className={`shadow-lg border-primary/20 h-full flex flex-col ${
          isLocked ? "opacity-50" : "hover:border-primary transition-colors"
        }`}
      >
        <CardHeader>
        ...
        <CardContent className="flex-grow flex flex-col justify-end space-y-4">
          ...
          {isLocked && (
            <p className="text-sm text-red-500 mt-2">
              {t("Esta unidad está bloqueada hasta que completes la anterior.")}
            </p>
          )}
        </CardContent>
      </Card>
