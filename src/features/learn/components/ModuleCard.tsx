    ...
    <div className="relative flex items-center justify-center mb-12">
      ...
      <div className="absolute -left-6 md:-left-10 z-10">
        <div className={`w-12 h-12 rounded-full flex items-center justify-center 
          ${module.isCompleted ? 'bg-green-500' : isLocked ? 'bg-gray-400' : 'bg-primary'} 
      ...
      <Card className={`w-full max-w-3xl mx-auto shadow-lg transition-all duration-300 
        ${isLocked ? 'opacity-60 cursor-not-allowed' : 'hover:shadow-xl hover:scale-[1.01]'} 
        ${module.isCompleted ? 'border-green-500' : 'border-primary/20'}`}>
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-primary flex items-center">
            {module.name}
            {module.isCompleted && <span className="ml-2 text-sm text-green-600">({t("Completado")})</span>}
          </CardTitle>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            ...
            <span>{t("Progreso")}: <span className="font-semibold text-primary">{module.progress}%</span></span>
          </div>
          <Progress value={module.progress} className="w-full h-2" />
          
          <div className="flex justify-end">
            <Button asChild disabled={isLocked} className="text-lg py-3">
              <Link to={module.url}>
