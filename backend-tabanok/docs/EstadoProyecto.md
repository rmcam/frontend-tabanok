...
*   **Autenticación:** Implementada con JWT y gestionada a través de cookies HttpOnly. Las rutas están protegidas globalmente con guards (`JwtAuthGuard`, `RolesGuard`). El manejo de tokens expirados está implementado. **Se ha implementado la verificación de tokens de refresco revocados en la función `refreshTokens`. Los tests unitarios para `auth.service.spec.ts` han sido corregidos y ahora pasan.** Ver [`./Autenticacion.md`](./Autenticacion.md) para más detalles.
