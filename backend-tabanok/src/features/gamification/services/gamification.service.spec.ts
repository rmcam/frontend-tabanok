  ...
  findOne: jest.fn().mockImplementation((query) => {
    ...
    if (!isNaN(userId) && userId >= 0 && userId < 100) {
      return {
        ...
        createdAt: new Date(),
        updatedAt: new Date(),
      } as any; // Eliminar gameStats de User mock
    }
    return undefined;
  }),
...
describe("GamificationService", () => {
  ...
  describe("addPoints", () => {
    it("should add points to a user and update their level", async () => {
      ...
      expect(userLevelRepository.save as jest.Mock).toHaveBeenCalledWith(
        result
      ); // Check if the UserLevel object was saved
      ...
      describe("Performance Testing", () => {
        ...
        it("should create UserLevel if it does not exist", async () => {
          ...
          expect(userLevelRepository.create as jest.Mock).toHaveBeenCalledWith({
            user: user,
          }); // Check if UserLevel was created
      ...
      describe("updateStats", () => {
        it("should update user game stats", async () => {
          ...
          expect(result.lessonsCompleted).toEqual(
            statsToUpdate.lessonsCompleted
          );
      ...
      describe("getUserStats", () => {
        it("should return user game stats including calculated level", async () => {
          ...
          (userLevelRepository.findOne as jest.Mock).mockResolvedValue(
            userLevel
          ); // Mock findOne on UserLevelRepository
      ...
      describe("grantAchievement", () => {
        it("should grant an achievement to a user", async () => {
          ...
          expect(
            userAchievementRepository.create as jest.Mock
          ...
          expect(
            userAchievementRepository.save as jest.Mock
      ...
      describe("grantBadge", () => {
        ...
        it("should throw an error if user is not found", async () => {
          ...
          // Act & Assert
          await expect(
            service.grantBadge(userId, badgeId)
          ).rejects.toThrowError(`User with ID ${userId} not found`);
          expect(userRepository.findOne).toHaveBeenCalledWith({
            where: { id: userId.toString() },
          });
        ...
        it("should throw an error if badge is not found", async () => {
          ...
          await expect(
            service.grantBadge(userId, badgeId)
          ).rejects.toThrowError(`Reward with ID ${badgeId} not found`);
          expect(userRepository.findOne).toHaveBeenCalledWith({
            where: { id: userId.toString() },
          });
          expect(rewardRepository.findOne).toHaveBeenCalledWith({
      ...
      describe("assignMission", () => {
        it("should assign a mission to a user", async () => {
          ...
          expect(
            userMissionRepository.create as jest.Mock
          ).toHaveBeenCalledWith(
            expect.objectContaining({
              userId: user.id,
      ...
      describe("awardPoints", () => {
        it("should award points and create an activity for a user", async () => {
          ...
          expect(userLevelRepository.save as jest.Mock).toHaveBeenCalledWith(
            result
          ); // Check if the UserLevel object with updated stats was saved
          expect(result).toBeInstanceOf(UserLevel); // Check the returned type
        });
        ...
        it("should update lesson completed stat for lesson activity type", async () => {
          ...
          expect(result.points).toEqual(
            initialUserLevel.points + pointsToAward
          );
          expect(result.experience).toEqual(
            initialUserLevel.experience + pointsToAward
          );
      ...
      it("should correctly grant a badge after completing an activity that meets the criteria", async () => {
        ...
        expect(userLevelRepository.save as jest.Mock).toHaveBeenCalledWith(
          updatedUserLevel
        ); // UserLevel saved after points/experience update
      ...
      it("should handle a large number of requests without performance degradation", async () => {
        ...
        const mockUser = (id: number) =>
          ({
            ...
            firstName: "Test",
            lastName: "User",
            roles: [UserRole.USER], // Añadir roles
            status: UserStatus.ACTIVE,
            languages: [],
            preferences: {
              notifications: false,
            ...
            culturalPoints: 0,
            level: 1, // Añadir level
            points: 0, // Añadir points
            gameStats: {}, // Añadir gameStats
            resetPasswordToken: undefined, // Añadir propiedades opcionales
            resetPasswordExpires: undefined,
            lastLoginAt: undefined,
            isEmailVerified: false,
            dateOfBirth: undefined,
    ...
    }); // Closing brace for Performance Testing describe block
  });
});
